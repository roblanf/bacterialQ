...
process_concat_test_tables <- function(log_list) {
  concat_test_tables <- map(log_list, ~{
    ...
    table <- table  %>% rename(model_string = Model) %>% 
      mutate(model_type = ifelse(str_detect(model_string, "^[a-zA-Z]__.*"), "trained", "existed"),
...
plot_BIC_trained_models <- function(concat_test_sum, output_folder) {
  plot_BIC_trained_model <- concat_test_sum %>% group_by(test) %>% filter(model_type == "trained" & F_type == "FO") %>% ungroup() %>% 
    ggplot(aes(x = max_tree_size, y = BIC)) + geom_line() +
    theme_light() + labs(title = "BIC of trained models on concatenated testing loci", x = "Maximum subtree size", y = "BIC")
...
plot_BIC_all_models <- function(concat_test_sum, output_folder) {
  trained_models_test <- concat_test_sum %>% filter(model_type == "trained") %>% mutate(model_string = str_c(model_type, "_", F_type))
  other_models_test <- concat_test_sum %>% filter(model_type == "existed") %>% group_by(model) %>% 
    filter(n() > 3) %>% ungroup()
  ...
  plot_BIC_all_models <- plot_all_models %>%
    ...
    theme_light() + labs(title = "BIC of all models on concatenated testing loci", x = "Maximum subtree size", y = "BIC")
...
plot_initial_model_distribution <- function(initial_model_table, output_folder) {
  plot_initial_model_distribution <- initial_model_table %>% 
    ...
    ggplot(aes(x = max_tree_size, y = count_ratio, group = model, color = model)) + geom_line() +
    theme_light() + labs(title = "Initial model distribution on different maximum subtree size", x = "Maximum subtree size", y = "Count ratio")

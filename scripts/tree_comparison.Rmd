...
phylograms <- function(tree, y_scale = -5, subtitle = "") {
  ggtree(tree, cex = 0.8, lwd = 0.3, aes(color = ifelse(is.na(as.numeric(label)), 100, as.numeric(label)))) +
...
if (result_df$nRF > 0.05) {
  ...
  cat("The normalized Robinson-Foulds distance is less than 0.02, cophylo plot will not be shown.")
}
```
